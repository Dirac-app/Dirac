...
function validateGmailResponse<T>(data: unknown, schema: z.ZodType<T>): T {
  const result = schema.safeParse(data);
  if (!result.success) {
    console.error("Gmail API response validation failed:", result.error.issues);
    throw new Error(`Gmail API response validation failed: ${result.error.issues[0]?.message}`);
  }
  return result.data;
}
...
async function gmailFetch(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (validateSchema) {
      return validateGmailResponse(jsonData, validateSchema);
    }
...
> {
  const data = await gmailFetch(
    ...
    undefined,
    GmailMessageListResponseSchema,
  );
  ...
  return batchConcurrent(
    ...
    async (stub: { id: string; threadId: string }) => {
