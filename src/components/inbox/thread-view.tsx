...
import type { DiracMessage } from "@/lib/types";
      ...
      <div className="flex h-[49px] items-center border-b border-border px-4">
        <div className="flex flex-1 items-center justify-between gap-3 min-w-0">
          ...
          <SnoozeButton threadId={thread.id} onSnooze={snoozeThread} />
            ...
            <ExportMenu thread={thread} messages={messages as DiracMessage[]} />
...
function ExportMenu({ thread, messages }: ExportMenuProps) {
  const [open, setOpen] = useState(false);
  ...
  const handleExport = async (format: "pdf" | "json" | "mbox") => {
    setExporting(true);
    try {
      // Fetch full thread data from the export API
      const res = await fetch(
        `/api/threads/export?platform=${platform}&threadId=${thread.id}`,
        { credentials: "include" },
      );
      if (!res.ok) throw new Error("Failed to fetch thread for export");

      const data = await res.json();
      const threadData = data.thread ?? data;
      const msgList: DiracMessage[] = data.messages ?? [];

      switch (format) {
        case "pdf":
          exportThreadAsPdf(threadData, msgList);
          break;
        case "json":
          exportThreadAsJson(threadData, msgList);
          break;
        case "mbox":
          exportThreadAsMbox(threadData, msgList);
          break;
      }
      ...
      setTimeout(() => { setExported(null); setOpen(false); }, 1800);
    } catch {
      toast({
        title: "Export failed",
        description: "Could not export this thread. Try again.",
        variant: "error",
      });
    } finally {
      setExporting(false);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <Tooltip>
        ...
        <TooltipContent>Export thread</TooltipContent>
      </Tooltip>
      <PopoverContent className="w-40 p-1" align="end">
        <p className="px-2 py-1.5 text-xs font-semibold text-muted-foreground">
          Export as…
        </p>
        {(["pdf", "json", "mbox"] as const).map((fmt) => (
          <button
            key={fmt}
            onClick={() => handleExport(fmt)}
            disabled={exporting}
            className="flex w-full items-center justify-between rounded px-2 py-1.5 text-left text-sm hover:bg-accent transition-colors disabled:opacity-50"
          >
            <span className="font-medium uppercase">{fmt}</span>
            {exported === fmt && <Check className="h-3 w-3 text-green-500" />}
          </button>
        ))}
      </PopoverContent>
    </Popover>
